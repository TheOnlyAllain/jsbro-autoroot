#!/bin/sh
# root_persistence.sh
# Logging to /tmp/jsbro-root.log

LOGFILE="/tmp/jsbro-root.log"
: > "$LOGFILE"
APPID="com.webos.service.secondscreen.gateway"
SCRIPT_NAME="Jsbro Root"
...
show_final_alert() {
    ...
    if [ "$outcome" = "success" ]; then
        ...
        if [ -n "${UPLOAD_LOG}" ] && log_url="$(upload_log)"; then
            ...
            base_msg="${base_msg} Check /tmp/jsbro-root.log for details."
        fi
    fi
...
log "===== jsbro-root finished successfully ====="
exit 0
